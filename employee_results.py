import os
import shutil
import pandas as pd
import sqlite3
import matplotlib.pyplot as plt

# Step 1: Copy the database folder and file
src_db_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'db', 'lesson.db'))
dst_db_dir = os.path.join(os.path.dirname(__file__), 'db')
dst_db_path = os.path.join(dst_db_dir, 'lesson.db')

if not os.path.exists(dst_db_dir):
    os.makedirs(dst_db_dir)
if not os.path.exists(dst_db_path):
    shutil.copy2(src_db_path, dst_db_path)

# Step 2: Connect to the database
conn = sqlite3.connect(os.path.join('..', 'db', 'lesson.db'))

# Step 3: Load the DataFrame using SQL
query = '''
SELECT last_name, SUM(price * quantity) AS revenue
FROM employees e
JOIN orders o ON e.employee_id = o.employee_id
JOIN line_items l ON o.order_id = l.order_id
JOIN products p ON l.product_id = p.product_id
GROUP BY e.employee_id;
'''
employee_results = pd.read_sql_query(query, conn)
conn.close()

# Step 4: Plotting
plt.figure(figsize=(10, 6))
plt.bar(employee_results['last_name'], employee_results['revenue'], color='skyblue')
plt.xlabel('Employee Last Name')
plt.ylabel('Revenue')
plt.title('Revenue by Employee')
plt.xticks(rotation=45)
plt.tight_layout()
plt.show()
